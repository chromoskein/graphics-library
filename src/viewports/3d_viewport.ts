...
export class Viewport3D {
  ...
  async render(frametime: number): Promise<void> {
    ...
    const gBufferOutputBindGroup = device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 3, resource: { buffer: this.gBuffer.globalsGPU } },
      ]
    });
    ...
    if (this.rayTraceGBuffer) {
      this.scene.renderRayTracingGBuffer({
        ...
        height: this.height,
        cameraBindGroup,
        outputBindGroup: gBufferOutputBindGroup,
